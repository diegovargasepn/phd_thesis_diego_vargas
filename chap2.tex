...
\begin{theorem}\label{thm:nemitski}
...
\begin{equation}\label{eq:coercivity_A}
...
\begin{equation}\label{eq:bilinear_form}    
    a(y,z):=\int_{\om}\l(\sum_{i,j=1}^{n} a_{ij}(x)\frac{\partial y(x)}{\partial x_i}\frac{\partial z(x)}{\partial x_{j}}\; dx+\sum_{i=1}^{n}b_i(x)\frac{\partial y(x)}{\partial x_i}z(x)+c(x)y(x)z(x) \r)\; dx.
\end{equation}
...
a(y,v)=(u,v)_{L^2(\om)}, \quad \mbox{for all }v\in H_0^1(\om)
\end{equation}

In addition, from $A^*$ we denote as $a^*$ its bilinear form defined similar as \eqref{eq:bilinear_form}. Recall 
that a function $v\in H_0^1(\om)$ is called a solution to the adjoint PDE equation associated to $A^*$ if 
...
Assuming that $A$ is coercive, $a_{ij}\in L^{\infty}(\om)$ and $c(x)\ge 0$ almost all $x\in\om$ it can 
be shown that the equation \eqref{eq:elliptic_pde} has a unique solution for any $u\in L^2(\om)$, 
see \cite[Remark 23]{brezis}. However, for results about regularity of solutions, it is necessary to introduce the concept of Lipschitz type domains.
...
\end{definition}
 ...
 \begin{theorem}\label{thm:existence_solution_elliptic_pde}
...
\subsection{Regularity of solutions}\label{subsection:regularity_pde_sol}
...
\begin{remark}\label{rmrk:operator_S}    
We finish this section showing the following result: thanks to Theorem 4.4 in 
\cite{stampacchia65} and the continuous injection $L^2(\om) \hookrightarrow W^{-1,p}(\om)$ for a fixed
 $p>n/2$, equation \eqref{eq:variational_formulation} admits a unique solution in $C(\bar\om)\cap H_0^1(\om)$. 
We denote the linear and continuous operator $S:L^2(\om)\to L^{\infty}(\om)\cap H_0^1(\om)$,
which assigns any $w\in L^2(\om)$  the corresponding state $y$  satisfying 
\eqref{eq:variational_formulation} with $w$ as right-hand side. 
The range of $S$ in $L^\infty(\om)$ complies better in our later discussions. 
The continuity of $S$ also implies the existence of a constant $c_{\infty}>0$ such that 
the solution $y=Sw$ of \eqref{eq:variational_formulation} satisfies
\begin{equation}\label{eq:esti_state_uniform}
    \norm{y}_{L^{\infty}(\om)}\le c_{\infty}\norm{w}_{L^2(\om),}
\end{equation}
Analogously, for the solution $z$ of \eqref{eq:variational_formulation*} it holds that 
...
The $\Gamma$-regularization, also referred to as the partial convexification of a 
nonconvex function, is a strategy commonly employed to deal with optimal control 
problems involving nonconvex penalizations; see, for example, 
\cite[Remark 2.6]{ito-kunisch2014}, \cite{casasl0}, and \cite{wachsmuth2019}. 
This approach is particularly relevant for penalization terms of the 
form $\int_{\om} |u(x)|^q\; dx$, with $q\in [0,1)$, which are of special interest
 in the present work. A detailed discussion of this strategy can be found in 
 \cite[Chapter 9]{ekeland}. The idea behind this approach is relatively simple. Suppose we consider a minimization problem of the form 
\[ 
...
\begin{equation}\label{eq:problem_biconjugate}
\begin{cases} \min_{u,y}\int_{\om}f^{**}(x, u(x), y(x))\; dx\\ \mbox{subejct to}\\
u=Sy \end{cases}
\end{equation}
...
\begin{lemma}
    ...
    \begin{equation}\label{eq:convex_envelope_phi}
    \phi_q^{**}(t)=\begin{cases}
        \phi_q(t), &|t|\ge t^*,\\
        \phi_{q}'(t^*)|t|, & |t|< t^*
    \end{cases}
...
The followig Proposition will be useful in Chapter 3. It is based on the properties of 
\eqref{eq:convex_envelope_phi}.
\begin{proposition}
    \label{prop:phi**_quadrature}
    The function $\phi_q^{**}$ defined in \eqref{eq:convex_envelope_phi} satisfies the inequality
    \[
    \phi_q^{**}(t)\ge \frac{\alpha}{2}|t|^2,
    \]
    for all $t\in \mathbb{R}$.
\end{proposition}
\begin{proof}
    When $|t|\ge t^*$, the result is inmediately. So, assume that $|t|<t^*$. Notice that
    \[
    \phi_{q}^{**}(t)\ge \frac{\alpha}{2}|t|^2\Leftrightarrow \l(\alpha t^*+\beta q (t^*)^{q-1}\r)|t|\ge \frac{\alpha}{2}|t|^2.
    \] 
    If $t=0$, the equality holds trivially. Otherwise, we have that
    \[
        \l(\alpha t^*+\beta q (t^*)^{q-1}\r)|t|\ge \frac{\alpha}{2}|t|^2\Leftrightarrow 2 t^*+\frac{2\beta q}{\alpha} (t^*)^{q-1}\ge |t|.
    \]
    Now,
    \begin{align*}
        2 t^*+\frac{2\beta q}{\alpha} (t^*)^{q-1}&=2t^*\l(1+\frac{\beta q}{\alpha}(t^{*})^{q-2}\r)\\
        &=2t^*\l(1+\frac{\beta q}{\alpha}\frac{\alpha}{2\beta (1-q)}\r)\\
        &=2t^*\l(1+\frac{q}{1-q}\r)\\
        &=2t^*\l(\frac{1}{1-q}\r)>t^*,\\
    \end{align*}
    since $0<q<1$. Therefore, as we assume $t^*>|t|$,
    $$
    2 t^*+\frac{2\beta q}{\alpha} (t^*)^{q-1}=2t^*\l(\frac{1}{1-q}\r)>t^*>|t|
    $$
    and thus the reuslt follows by multipliying the last expression by $|t|$.
\end{proof}
...
% --------- Parameters ----------
\pgfmathsetmacro{\alpha}{0.4}
\pgfmathsetmacro{\bpar}{1.6}
\pgfmathsetmacro{\q}{0.5}
\pgfmathsetmacro{\tstar}{2.5198420997897464}
\pgfmathsetmacro{\phiT}{(\alpha/2)*(\tstar)^2 + \bpar*(abs(\tstar))^(\q)}
\pgfmathsetmacro{\m}{\phiT/\tstar}
...
{(\alpha/2)*(x^2) + \bpar*(abs(x))^(\q)};

% --------- Greatest affine minorant ----------
\addplot [
    line width=2.2pt,
    blue,
    samples=300,
...
\addplot [dashed] coordinates {(-\tstar,0) (-\tstar,\phiT)};
\addplot [dashed] coordinates {(\tstar,0) (\tstar,\phiT)};

% --------- Points ----------
\addplot [only marks, mark=*, mark size=3pt] coordinates {(\tstar,\phiT)};
\addplot [only marks, mark=*, mark size=3pt] coordinates {(-\tstar,\phiT)};
\addplot [only marks, mark=*, mark size=3pt] coordinates {(-\tstar,0)};
\addplot [only marks, mark=*, mark size=3pt] coordinates {(\tstar,0)};

% --------- Labels ----------
\node at (axis cs:-3.1,0.3) {$-t^*$};
\node at (axis cs:2.3,0.3) {$t^*$};

\node at (axis cs:-2.6,6.1) {$l_1(t)$};
\node at (axis cs:-5.2,5.2) {$l_2(t)$};

\node at (axis cs:-2.5,4.3) {$A$};
\node at (axis cs:2.2,4.3) {$B$};
...
\pgfmathsetmacro{\beta}{1.6}
\pgfmathsetmacro{\tstar}{2.5298221281347035}
\pgfmathsetmacro{\phiT}{(\alpha/2)*(\tstar)^2 + \beta}
\pgfmathsetmacro{\m}{sqrt(2*\alpha*\beta)}
...
% Labels
\node at (axis cs:-3.0,0.3) {$-t_0^*$};
\node at (axis cs:2.3,0.3) {$t_0^*$};
...
The theory of Difference of Convex  (DC) functions was developed in the later 70's by Toland first, 
and Singer \cite{toland, singer}, in the context of convex maximization theory in finite dimensional 
spaces. Later, several deep results and algorithms based on this theory was proposed by Pham Dinh Tao 
and Le Thi Hoai An in \cite{tao}. Inspired by this work, the infinite dimensional case for finding 
existence of solutions, necessary and sufficient optimal conditions of DC problems in the context of 
indefinite quadratic problems are deveolped in \cite{bonnans, yen}. The later novelty paper \cite{lim} deals with DC algorithms in general Hilbert spaces, proving sufficient conditions for the convergence of this algorithm and given a rate of convergence for indefinite quadratic programs. 
